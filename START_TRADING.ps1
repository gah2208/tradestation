...
# Revision: 2.1
# v2.1 - Banner title updated from CTV2 to VTB (user-facing branding).
# v2.0 - Copyright (c) Gregory Howard 2026 All rights reserved added to
#         console banner. Preflight call updated to --mode start so
#         preflight_check.py logs acceptance as "Start Trading".
# v1.9 - Clear live_confirmed.flag whenever runner exits (clean or crash).
#         Live trading always requires fresh confirmation on next start.
# v1.7 - Added -Live switch. Default is sandbox (paper trading).
#         Use -Live to connect to live Tradier account.
#         Mode label and log filename now reflect sandbox vs live.
# v1.6 - Added -u flag to Python call to force unbuffered output through Tee-Object
# v1.5 - Log redirection: stdout+stderr tee'd to C:\CTV\logs\ctv2_YYYYMMDD_HHMMSS.log
#         Terminal stays open after exit/crash via Read-Host. Logs dir auto-created.
# v1.4 - Removed ctv_files integrity block; manifest now checks python + tws only
# v1.3 - Removed config_ctv2.py integrity block
# v1.2 - Fixed undefined $CTV_DIR variable in config backup path
# v1.1 - ConvertTo-Json -Depth 3 -> 5 (prevents ctv_files truncation in manifest)
# ============================================================
param(
    [switch]$Live
)
...
function Invoke-IntegrityCheck {
    ...
    if (-not (Test-Path $manifestPath)) {
        ...
        Write-Host "  Run CTV_SETUP.ps1 to generate a fresh manifest before trading."
        Write-Host ""
        return $false
    }
...
python preflight_check.py --mode start
if ($LASTEXITCODE -ne 0) {
    Write-Host ""
    Write-Host "  Setup cancelled. Trading not started."
    Write-Host ""
    Read-Host "Press Enter to exit"
    exit 1
}
...
# Run runner with stdout+stderr captured to log and echoed to terminal
if ($Live) {
...
# Clear live trading confirmation - always require fresh confirmation on next start
